	...
	function (declare, _WidgetBase, _TemplatedMixin, dom, on, dojoClass, dojoStyle, dojoConstruct, dojoAttr, dojoLang, dojoHtml, dojoArray, widgetTemplate) {
		...
		return declare("SimpleCheckboxSetSelector.widget.SimpleCheckboxSetSelector", [_WidgetBase, _TemplatedMixin], {
			...
			postCreate: function () {
				...
				this._entity = this.CheckboxOptionObject;
				this._reference = this.entity.split("/")[0];
			...
			_populateCheckboxOptions: function (objs) {
				...
				for (var i = 0; i < objs.length; i++) {
					var mxObj = objs[i];
					optionLabel = mxObj.get(this.displayAttribute);
          label = (mxObj.get(this.defaultAttr)) ? optionLabel + " <i><strong>(" + this.defaultCaption + ")</strong></i>" : optionLabel;
					...
					this._checkboxOptions[mxObj.getGuid()] = {
						obj: mxObj,
						label: label
					}
          ...
          freeTextValue = null,
				...
				for (var o = 0; o < this._checkboxOptionsArray.length; o++) {
					...
					if (option.value) {
						...
						if (this._checkboxOptionsArray[o].freetext) {
								freeTextValue = this._checkboxOptionsArray[o].obj.get(this.freeTextAttr);
								freeTextNode = this._createFreeTextNode(option, this._checkboxOptionsArray[o], freeTextValue);
								this._checkboxOptions[this._checkboxOptionsArray[o].guid].freetextnode = freeTextNode;
								dojoConstruct.place(freeTextNode, labelNode, "last");
								this.connect(freeTextNode, "onkeyup", dojoLang.hitch(this, this.eventOnChange, freeTextNode, this._checkboxOptionsArray[o].obj));
						}
			...
			_createFreeTextNode: function(key, option, value) {                
					var display = (this._contextObj.get(this._reference).indexOf(key.guid) !== -1);
			...
			_addOnclickToCheckboxItem: function (checkboxNode, rbvalue) {
				...
				this.connect(checkboxNode, "onclick", dojoLang.hitch(this, function () {
					...
					if (this._checkboxOptions[rbvalue].freetextnode) {
						if (checkboxNode.checked) {
							dojoStyle.set(this._checkboxOptions[rbvalue].freetextnode, "display", "block");
						} else {
							dojoStyle.set(this._checkboxOptions[rbvalue].freetextnode, "display", "none");
						}
